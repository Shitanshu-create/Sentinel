import { getDepartmentForCommander, getUnitsSummaryForDepartment, getUnitDetail } from "../services/commandingOfficer.service.js";
import OrganizationalNote from "../models/organizationalNote.model.js";

async function getUnitsController(req, res) {
    try {
        const department = await getDepartmentForCommander(req.user.Id);
        const units = await getUnitsSummaryForDepartment(department);
        res.status(200).json({ message: "Unit summary retrieved successfully", department, units });
    } catch (error) {
...
async function getUnitDetailController(req, res) {
    try {
        ...
        const detail = await getUnitDetail(department, unitName);

        if (!detail) {
            return res.status(404).json({ message: "Unit not found in your department" });
        }

        const notes = await OrganizationalNote.find({ department, unit: unitName }).sort({ createdAt: -1 });

        res.status(200).json({ message: "Unit detail retrieved successfully", ...detail, notes });
    } catch (error) {
