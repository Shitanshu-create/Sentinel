...
function Register({ onBack, onOpenLogin, onRegisterSuccess }) {
  ...
  const handleNextStep1 = () => {
    setError(null);
    const res = validateStep1PersonalDetails(personalDetails);
    if (res.message) {
  ...
  const handleFinalSubmit = async () => {
    setError(null);
    const valStep1 = validateStep1PersonalDetails(personalDetails);
    if (valStep1.message) {
      setError(valStep1.message);
      setCurrentStep(1);
      return;
    }
    ...
    setLoading(true);
    const res = await handleRegister(fullPayload);
    setLoading(false);
        ...
        <section className="auth-sketch-card auth-card-padding">
          ...
          {currentStep === 1 && (
            <PersonalDetailsStep
              ...
              updateData={(fields) => setPersonalDetails((prev) => ({ ...prev, ...fields }))}
          ...
          {currentStep === 1 && (
            <>
              ...
              <div className="auth-oauth-group">
                <a href={`${env.apiBaseUrl}/api/auth/google`} className="auth-oauth-btn">
                ...
                <a href={`${env.apiBaseUrl}/api/auth/github`} className="auth-oauth-btn">
